...
const HistoricoCoordenadores = () => {
  ...
  const gerarPDF = () => {
    ...
    if (avaliacoesFiltradas.length === 0) {
      ...
      return doc.save(`Relatório_${tipoSelecionado}_${unidadeSelecionada}_${mesSelecionado}.pdf`);
    }
    ...
    avaliacoesFiltradas.forEach((avaliacao, index) => {
      ...
      // Info básica
      const info = [
        { label: "Data", valor: avaliacao.date ? new Date(avaliacao.date).toLocaleDateString("pt-BR") : "N/A" },
      ...
      if (avaliacao.comentarios?.trim()) {
        ...
        doc.setFont("helvetica", "normal");
        const comentariosLinhas = doc.splitTextToSize(avaliacao.comentarios, 170);
      ...
      const competencias = [
        ...
        ["Aplicação Habilidades e Competências", avaliacao.aplicacao_habilidades_competencias],
        ["Construção Pensamento", avaliacao.construcao_pensamento],
        ["Postura Professor", avaliacao.postura_professor],
        ["Divisão Tempo Teoria e Prática", avaliacao.divisao_tempo_teoria_pratica],
      ...
      if (competencias.length > 0) {
        ...
        competencias.forEach(([titulo, valor]) => {
          ...
          else if (valor === "Atendeu parcialmente") doc.setTextColor(0, 102, 204);
      ...
      // Rodapé
      doc.setFontSize(9);
      doc.text("software IP Control Gestão - Todos os direitos reservados.", pageWidth / 2, pageHeight - 10, {
        align: "center"
      });
    });
  
    doc.save(`Relatório_${tipoSelecionado}_${unidadeSelecionada}_${mesSelecionado}.pdf`);
  };
      ...
      {mesSelecionado && avaliacoesFiltradas.length > 0 && (
        <div className="mt-4 p-4 border rounded-md bg-gray-100">
          ...
          {avaliacoesFiltradas.map((avaliacao, index) => (
            ...
            {avaliacao.regente && (
              <p><strong>Regente:</strong> {avaliacao.regente}</p>
            )}
            ...
            {avaliacao.conformidade && (
              <p><strong>Conformidade:</strong> {avaliacao.conformidade}</p>
            )}
